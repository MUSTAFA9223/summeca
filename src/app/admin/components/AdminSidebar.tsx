        ...
        {!collapsed && (
          <div className="flex items-center gap-2.5 px-3 py-2 mb-1">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-muted-foreground truncate" data-ltr>{adminEmail}</div>
            </div>
          </div>
        )}
...
export default function AdminSidebar({ collapsed, onToggleCollapse, mobileOpen, onCloseMobile, adminName, adminEmail }: AdminSidebarProps) {
  return (
    <>
      <aside data-admin-sidebar="desktop" className={`fixed left-0 top-0 bottom-0 z-40 hidden lg:flex flex-col bg-card border-r border-border transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'}`}>
      ...
      <aside
        ...
        className={`fixed left-0 top-0 bottom-0 z-40 lg:hidden flex flex-col bg-card border-r border-border w-64 transition-transform duration-300 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="flex items-center justify-between h-14 border-b border-border px-4">
          ...
          <button onClick={onCloseMobile} aria-label="Close" className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-secondary transition-all">✕</button>
        </div>
