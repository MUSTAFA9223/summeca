    ...
    {!pageLoading && cartItem && <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
      <div className="lg:col-span-3 space-y-5">
        ...
        {cryptoSession && <div className="bg-card border border-primary/30 rounded-2xl p-5 space-y-4">
          <div className="flex items-center gap-2"><CheckCircle2 size={18} className="text-success" /><h2 className="font-700">Crypto payment created</h2></div>
          <div><p className="text-xs text-muted-foreground mb-1">Amount to send</p><p className="font-800 text-lg">{cryptoSession.cryptoAmount}</p></div>
          ...
          <div className="p-3 rounded-xl bg-warning/5 border border-warning/20 text-xs">Send only the selected asset on the selected network. Do not close the payment as completed yourself; SUMMECA waits for the provider webhook.</div>
      ...
      <div className="lg:col-span-2"><div className="bg-card border border-border rounded-2xl sticky top-24 p-5">
        ...
        <button onClick={() => void handleCheckout()} disabled={authLoading || checkoutSubmitting || (!isFreeOrder && !selectedProviderAvailable) || Boolean(cryptoSession)} className="mt-4 w-full py-3.5 bg-gradient-teal text-white font-700 text-sm rounded-xl disabled:opacity-50 flex items-center justify-center gap-2">
          {checkoutSubmitting ? <><Loader2 size={16} className="animate-spin" /> Processing…</> : !user ? 'Sign In to Continue' : isFreeOrder ? <><CheckCircle2 size={15} /> Get Free Access</> : checkoutMethod === 'crypto' ? <><Bitcoin size={15} /> Create Crypto Payment</> : <><ExternalLink size={14} /> Pay with Payoneer</>}
        </button>
