...
const CATEGORY_FEATURES = [
  { icon: Download, title: 'Instant Download', desc: 'Get your files immediately after purchase, no waiting' },
  { icon: Layers, title: 'Ready-to-Use', desc: 'Professionally designed templates you can customize fast' },
  { icon: Database, title: 'Curated Datasets', desc: 'Clean, structured data for training models and research' },
];
...
function ProductCard({ product, featured = false }: { product: Product; featured?: boolean }) {
  ...
  const badge = TYPE_BADGE[product.category] ?? { label: 'Digital', cls: 'bg-warning/10 text-warning' };

  return (
    <Link
      href={`/products/${product.slug}`}
      className={`group rounded-2xl border border-border p-5 card-hover cursor-pointer block bg-gradient-card-digital ${
    ...
    >
      <div className="flex items-start justify-between mb-4">
        <div className="w-11 h-11 rounded-xl bg-white shadow-card flex items-center justify-center flex-shrink-0">
          ...
          <FileText size={20} className="text-warning" style={product.thumbnail_url ? { display: 'none' } : {}} />
        </div>
        ...
        <div>
          {lowestPrice === 0 || lowestPrice === null ? (
            ...
            <span className="text-sm font-700 text-foreground tabular-nums">
              ...
              <span className="text-xs font-400 text-muted-foreground"> once</span>
            </span>
          )}
        </div>
        <span className="text-xs font-600 text-primary group-hover:underline">
          {lowestPrice === 0 || lowestPrice === null ? 'Download Free →' : 'View →'}
        </span>
      </div>
    </Link>
  );
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <section className="pt-24 pb-14 bg-gradient-hero border-b border-border relative overflow-hidden">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute top-10 right-20 w-72 h-72 rounded-full bg-warning/5 blur-3xl" />
          <div className="absolute bottom-0 left-10 w-48 h-48 rounded-full bg-info/5 blur-2xl" />
        </div>
        <div className="max-w-screen-xl mx-auto px-6 lg:px-8 relative">
          <div className="flex items-center gap-2 text-xs text-muted-foreground mb-6">
            ...
            <span>/</span>
            <span className="text-foreground font-600">Digital Products</span>
          </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-8">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-warning/10 border border-warning/20 mb-4">
                ...
                <span className="text-xs font-700 text-warning uppercase tracking-widest">Digital Products</span>
              </div>
              <h1 className="text-4xl lg:text-5xl font-800 text-foreground mb-4 leading-tight">
                Templates & datasets,{' '}
                <span className="text-gradient-primary">ready to use</span>
              </h1>
              <p className="text-base text-secondary-foreground leading-relaxed max-w-xl">
                Download professionally crafted templates, curated datasets, and digital assets. One-time purchase, instant access — no subscriptions required.
              </p>
            </div>
        ...
        <section className="py-10 border-b border-border bg-white/50">
          <div className="max-w-screen-xl mx-auto px-6 lg:px-8">
            <div className="flex items-center justify-between mb-6">
              ...
              <Link href="/products?filter=digital" className="text-xs font-600 text-primary hover:text-primary/80 flex items-center gap-1 transition-colors">
        ...
        <div className="max-w-screen-xl mx-auto px-6 lg:px-8">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 py-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-700 text-foreground">All Digital Products</span>
              {!loading && (
      ...
      <main className="flex-1 max-w-screen-xl mx-auto px-6 lg:px-8 py-10 w-full">
          ...
          <div className="flex items-center justify-between mb-6">
            <p className="text-sm text-muted-foreground">
              {sorted.length === 0
                ...
                : `${sorted.length} digital product${sorted.length !== 1 ? 's' : ''} found`}
            </p>
            {search && (
              <button
                onClick={() => setSearch('')}
        ...
        {!loading && !error && sorted.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-base font-600 text-foreground mb-1">No digital products match your search</p>
      ...
      <section className="py-14 border-t border-border bg-gradient-hero">
        <div className="max-w-screen-xl mx-auto px-6 lg:px-8 text-center">
          ...
          <p className="text-sm text-secondary-foreground mb-6 max-w-md mx-auto">
            Browse AI tools and SaaS apps in our full marketplace.
          </p>
